        ...
        try {
            ...
            if (orders.length > 0) {
                ...
                if (validConditionalOrders.length > 0) {
                    ...
                    const staticCalls = validConditionalOrders.map(order => {
                        // incrementOrderRetries(order.account, order.conditionalOrderId, order.retries);
                    ...
                    // Perform static call to check which orders can be executed
                    const staticResults = await multicall.callStatic.aggregate3(staticCalls);

                    console.log('staticResults', staticResults);

                    // Filter the executable orders based on static call results
                    const successfulOrders = validConditionalOrders.filter((_, index) => staticResults[index].success);

                    logger.info(`Total Successful Orders: ${successfulOrders.length}`);
